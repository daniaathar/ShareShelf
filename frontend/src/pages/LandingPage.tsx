...
import { ArrowRight, Camera, Drill, Search, Sparkles, Tent, Wrench, ShieldCheck, Zap, HandCoins } from "lucide-react";
...
const featured = [
  { title: "Cordless drill kit", price: 8, city: "Brooklyn, NY", img: "https://images.unsplash.com/photo-1504148455328-c376907d081c?w=800&auto=format&fit=crop&q=60" },
  { title: "4-person camping tent", price: 22, city: "Austin, TX", img: "https://images.unsplash.com/photo-1504280390367-361c6d9f38f4?w=800&auto=format&fit=crop&q=60" },
  { title: "Sony A7 III camera", price: 45, city: "San Francisco, CA", img: "https://images.unsplash.com/photo-1519638831568-d9897f54ed69?w=800&auto=format&fit=crop&q=60" },
  { title: "Party sound system", price: 30, city: "Chicago, IL", img: "https://images.unsplash.com/photo-1519389950473-47ba0277781c?w=800&auto=format&fit=crop&q=60" },
];
    ...
    <div>
        ...
        <div className="relative mx-auto max-w-7xl px-4 pb-20 pt-16 sm:px-6 sm:pt-24 lg:px-8">
          ...
          >
            ...
            <p className="mx-auto mt-5 max-w-2xl text-pretty text-base text-muted-foreground sm:text-lg">
              ShareShelf is a peer-to-peer marketplace for tools, gear, and everyday items.
              Skip the store — borrow it from a neighbor.
            </p>
            ...
            >
              ...
              <Button className="rounded-full bg-[image:var(--gradient-hero)] px-6 text-primary-foreground">
                Search
              </Button>
            </form>

            <div className="mt-6 flex flex-wrap justify-center gap-2 text-xs text-muted-foreground">
              {["Drill", "Projector", "Stroller", "Pressure washer", "Party lights"].map((t) => (
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <div className="flex items-end justify-between">
          ...
          <Link to="/" className="hidden text-sm font-medium text-primary hover:underline sm:block">
          ...
          {featured.map((f, i) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <div className="mt-1 text-xs text-muted-foreground">{f.city}</div>
                <div className="mt-3 flex items-baseline justify-between">
                  <span className="text-lg font-semibold">${f.price}<span className="text-xs text-muted-foreground"> / day</span></span>
      ...
      <section className="mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8">
        <SectionHeader eyebrow="How it works" title="Three steps to rent (or earn)" center />
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {[
            { icon: Search, title: "Find it nearby", desc: "Search for the item you need in your city and neighborhood." },
            { icon: ShieldCheck, title: "Book with trust", desc: "Verified profiles, secure deposits, and transparent reviews." },
            { icon: Zap, title: "Pick up & go", desc: "Coordinate pickup with the owner — usually the same day." },
          ].map((s, i) => (
            <div key={s.title} className="relative rounded-3xl border border-border bg-card p-8">
      ...
      <section className="mx-auto max-w-7xl px-4 pb-24 sm:px-6 lg:px-8">
        <div className="relative overflow-hidden rounded-3xl bg-[image:var(--gradient-hero)] p-10 text-primary-foreground sm:p-14">
          ...
          <div className="relative flex flex-col items-start justify-between gap-6 sm:flex-row sm:items-center">
            <div>
              <h3 className="text-2xl font-semibold sm:text-3xl">Turn your shelf into income</h3>
              <p className="mt-2 max-w-xl text-sm text-primary-foreground/80">
                List your unused items in minutes and earn every time a neighbor borrows.
              </p>
            </div>
            <Button asChild size="lg" className="rounded-full bg-background text-foreground hover:bg-background/90">
              <Link to="/register">List an item <ArrowRight className="ml-1 h-4 w-4" /></Link>
            </Button>
          </div>
        </div>
      </section>
